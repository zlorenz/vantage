<?php
/**
* Plugin Name: SiliconFolio Pheromone Edition
...
add_action( 'init', 'pheromone_load_textdomain' );

function pheromone_load_textdomain() {
...
function st_sf_portfolio() {
	register_post_type( 'portfolio',
		array(
			'labels' => array(
				'name' => __( 'Portfolio', 'pheromone' ),
				'singular_name' => __( 'Portfolio', 'pheromone' ),
				'new_item' => __( 'Add New portfolio', 'pheromone' ),
				'add_new_item' => __( 'Add New portfolio', 'pheromone' )
			),
...
function extra_fields_plugins() {
	add_meta_box( 'extra_fields_plugin', 'Portfolio Additional Settings', 'extra_fields_for_portfolio', 'portfolio', 'normal', 'low'  );
...
function extra_fields_for_portfolio( $post ){ ?> 
    <div style="padding:20px; border:1px solid #eee; background:#f7f7f7;margin:30px 20px;">    
    	<p><b>Some Additional Options</b></p>
		<hr>
	    <h4>Client:</h4>
	    <input type="text" name="extra[port-client]" placeholder="Google" value="<?php echo esc_attr(get_post_meta($post->ID, 'port-client', true)); ?>" />

	    <h4>Date:</h4>
	    <input type="text" name="extra[port-date]" placeholder="August 17, 2017" value="<?php echo esc_attr(get_post_meta($post->ID, 'port-date', true)); ?>" />

	    <h4>Service:</h4>
	    <input type="text" name="extra[port-service]" placeholder="Design" value="<?php echo esc_attr(get_post_meta($post->ID, 'port-service', true)); ?>" />

	    <h4>Video For Single Page:</h4>
	    <input type="text" name="extra[port-video]" style="width:500px;" placeholder="iframe or embed code" value="<?php echo esc_attr(get_post_meta($post->ID, 'port-video', true)); ?>" />
...
?>
    <div style="padding:20px; border:1px solid #eee; background:#f7f7f7;margin:30px 20px;">    
   	<p><b>Use only with Page Template: Portfolio</b></p>
	<hr>
    ...
    "rtws"  => array("name" => "Masonry Grid With Spaces"),
    "rtwos"  => array("name" => "Masonry Grid Without Spaces"),
	"fsqws"  => array("name" => "4 Columns With Spaces"),
	"fsqwos"  => array("name" => "4 Columns Without Spaces"),
	"sqws"  => array("name" => "3 Columns With Spaces"),
	"sqwos"  => array("name" => "3 Columns Without Spaces"),
	"htws"  => array("name" => "2 Columns With Spaces"),
	"htwos"  => array("name" => "2 Columns Without Spaces"),
