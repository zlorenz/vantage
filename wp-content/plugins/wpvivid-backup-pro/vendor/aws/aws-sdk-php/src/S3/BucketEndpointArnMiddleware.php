...
use WPvividProAws\Arn\ArnParser;
use WPvividProAws\Arn\ObjectLambdaAccessPointArn;
use WPvividProAws\Arn\Exception\InvalidArnException;
use WPvividProAws\Arn\AccessPointArn as BaseAccessPointArn;
use WPvividProAws\Arn\S3\OutpostsAccessPointArn;
use WPvividProAws\Arn\S3\MultiRegionAccessPointArn;
use WPvividProAws\Arn\S3\OutpostsArnInterface;
...
use WPvividProAws\Exception\UnresolvedEndpointException;
...
class BucketEndpointArnMiddleware
{
    ...
    private function generateAccessPointHost(
        ...
        $region = \WPvividProAws\strip_fips_pseudo_regions($region);
    ...
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if (!($clientPart->isRegionMatch($this->region, 's3'))) {
                $clientPart = $this->partitionProvider->getPartition(
                    \WPvividProAws\strip_fips_pseudo_regions($this->region),
                    's3'
                );
            }
            if (!$arn instanceof MultiRegionAccessPointArn) {
