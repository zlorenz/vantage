...
class Transfer implements PromisorInterface
{
    ...
    private function createDownloadPromise()
    {
        ...
        return (new WPvividProAws\CommandPool($this->client, $commands, [
    ...
    private function createUploadPromise()
    {
        ...
        $files = \WPvividProAws\map($this->getUploadsIterator(), function ($file) {
    ...
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return WPvividProAws\filter(
                WPvividProAws\recursive_dir_iterator($this->sourceMetadata['path']),
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = $this->client
                ...
                ->search('Contents[].Key');
            $files = WPvividProAws\map($files, function ($key) use ($listArgs) {
                return "s3://{$listArgs['Bucket']}/$key";
            });
            return WPvividProAws\filter($files, function ($key) {
