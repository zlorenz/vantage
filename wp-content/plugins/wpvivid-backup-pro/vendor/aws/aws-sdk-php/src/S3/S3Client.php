...
use WPvividProAws\Api\DocModel;
use WPvividProAws\Api\Service;
use WPvividProAws\AwsClient;
use WPvividProAws\CacheInterface;
use WPvividProAws\ClientResolver;
use WPvividProAws\Command;
use WPvividProAws\Exception\AwsException;
use WPvividProAws\HandlerList;
use WPvividProAws\InputValidationMiddleware;
use WPvividProAws\Middleware;
use WPvividProAws\Retry\QuotaManager;
use WPvividProAws\RetryMiddleware;
use WPvividProAws\ResultInterface;
use WPvividProAws\CommandInterface;
use WPvividProAws\RetryMiddlewareV2;
use WPvividProAws\S3\UseArnRegion\Configuration;
use WPvividProAws\S3\UseArnRegion\ConfigurationInterface;
use WPvividProAws\S3\UseArnRegion\ConfigurationProvider as UseArnRegionConfigurationProvider;
use WPvividProAws\S3\RegionalEndpoint\ConfigurationProvider;
...
class S3Client extends AwsClient implements S3ClientInterface
{
    ...
    public function createPresignedRequest(CommandInterface $command, $expires, array $options = [])
    {
        ...
        return $signer->presign(
            \WPvividProAws\serialize($command),
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \WPvividProAws\serialize($command)->getUri();
    }
    ...
    {
        if ($value) {
            $config = \WPvividProAws\Retry\ConfigurationProvider::unwrap($value);
