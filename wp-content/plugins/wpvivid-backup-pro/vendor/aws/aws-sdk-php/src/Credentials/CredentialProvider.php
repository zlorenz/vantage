<?php
namespace WPvividProAws\Credentials;

use WPvividProAws;
use WPvividProAws\Api\DateTimeResult;
use WPvividProAws\CacheInterface;
use WPvividProAws\Exception\CredentialsException;
use WPvividProAws\Sts\StsClient;
...
class CredentialProvider
{
    ...
    public static function sso($ssoProfileName, $filename = null, $config = [])
    {
        ...
        return function () use ($ssoProfileName, $filename, $config) {
            ...
            if (empty($config['ssoClient'])) {
                $ssoClient = new WPvividProAws\SSO\SSOClient([
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use ($profile, $filename) {
            ...
            $data = \WPvividProAws\parse_ini_file($filename, true, INI_SCANNER_RAW);
            if ($data === false) {
                return self::reject("Invalid credentials file: $filename");
            }
            if (!isset($data[$profile])) {
    ...
    {
        $profileData = \WPvividProAws\parse_ini_file($filename, true, INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials'
            ...
            $configProfileData = \WPvividProAws\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles() {
        ...
        if (file_exists($credFile)) {
            $profiles = \WPvividProAws\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }

        if (file_exists($configFile)) {
            $configProfileData = \WPvividProAws\parse_ini_file($configFile, true, INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
