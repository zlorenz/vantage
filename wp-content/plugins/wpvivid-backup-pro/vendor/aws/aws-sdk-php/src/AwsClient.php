...
use WPvividProAws\ClientSideMonitoring\ConfigurationProvider;
use WPvividProAws\EndpointDiscovery\EndpointDiscoveryMiddleware;
...
class AwsClient implements AwsClientInterface
{
    ...
    private function parseClass()
    {
        ...
        if ($klass === __CLASS__) {
            return ['', 'WPvividProAws\Exception\AwsException'];
        }
        ...
        return [
            strtolower($service),
            "WPvividProAws\\{$service}\\Exception\\{$service}Exception"
        ];
    }
    ...
    private function loadAliases($file = null)
    {
        if (!isset($this->aliases)) {
            ...
            $aliases = \WPvividProAws\load_compiled_json($file);
    ...
    {
        $aliases = \WPvividProAws\load_compiled_json(__DIR__ . '/data/aliases.json');
