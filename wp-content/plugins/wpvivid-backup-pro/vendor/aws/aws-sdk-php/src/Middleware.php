    ...
    public static function mapRequest(callable $f)
    {
        return function (callable $handler) use ($f) {
            return function (
                ...
                $modifiedRequest = $f($request);
                if (!($modifiedRequest instanceof RequestInterface)) {
                    throw new \UnexpectedValueException("Expected instance of RequestInterface");
                }
                return $handler($command, $modifiedRequest);
            };
        };
    }
    ...
    public static function mapCommand(callable $f)
    {
        return function (callable $handler) use ($f) {
            return function (
                ...
                $modifiedCommand = $f($command);
                if (!($modifiedCommand instanceof CommandInterface)) {
                    throw new \UnexpectedValueException("Expected instance of CommandInterface");
                }
                return $handler($modifiedCommand, $request);
            };
        };
    }
