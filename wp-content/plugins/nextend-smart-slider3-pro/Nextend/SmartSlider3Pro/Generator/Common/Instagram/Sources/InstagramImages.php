...
class InstagramImages extends AbstractGenerator {
    ...
    private function inAvailableMediaTypes($img, $force = false) {
        //$force is required, because children images should be shown while CAROUSEL_ALBUM is selected
        $mediaTypes = [
            'IMAGE'          => intval($force || $this->data->get('allow_images', 1)),
    ...
    private function transformData($img, $dateOptions, $parent = null, $withImage = true) {
        $data = array();
        if ($parent && isset($parent->caption)) {
            $data['caption'] = $parent->caption;
        } else if (isset($img->caption)) {
    ...
    private function iterateImages($images, $dateOptions, &$data, &$idx, $api, &$childrenIdx = null, $parent = null) {
        ...
        foreach ($images as $img) {
            ...
            if ($this->inAvailableMediaTypes($img, isset($parent))) {
                ...
                if ($img->media_type === 'CAROUSEL_ALBUM' && $this->data->get('allow_album', 0)) {
                    ...
                    if ($albumType) {
                        ...
                        $this->iterateImages($children->data, $dateOptions, $data, $idx, $api, $childrenIdx, $parent);
                        $parent      = null;
                        $childrenIdx = null;
                        $idx++;
                    } else {
                        $data[$idx] = $this->transformData($img, $dateOptions);
                        if (empty($data[$idx]['image'])) {
                            $children   = $api->getMediaChildren($img->id);
                            $data[$idx] = $this->transformData($children->data[0], $dateOptions, $parent);
                        }
                        $idx++;
                    }
                } else {
