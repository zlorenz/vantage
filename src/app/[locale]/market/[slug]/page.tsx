/**
 * Market taxonomy archive — filtered portfolio grid with pre-selected market.
 */
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    enPath: `/market/${term.slug}`,
    zhPath: `/zh/市场/${term.slugZh || term.slug}`,
...
export default async function MarketArchivePage({ params }: Props) {
  ...
  const pageUrl = marketPageUrl(typedLocale, term.slug, term.slugZh);
        ...
        <div className="container-fluid px-3 md:px-4">
          ...
          <Suspense fallback={<div className="vp-load-spinner" />}>
            <PortfolioGrid
              ...
              markets={markets}
              phrases={phrases}
              presetFilters={{ market: activeSlug }}
            />
          </Suspense>
        </div>
      </SectionWrapper>
